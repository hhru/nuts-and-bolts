...
- Подняли версию jackson до 2.6.6
- Добавили возможность указать host для Jetty
```

#### [3.5.0]
```
- Добавили HttpSessionManager для аутентификации
```

#### [3.4]
```
- Вынесли конфигурацию Launcher в ServletConfig
- Переименовали shared-logback.xml в nab-shared-logback.xml из-за пересечений в сервисах
```

#### [3.3]
```
- Добавлена поддержка нескольких data sources: MASTER, READONLY, SLOW
...
#### [3.2]
```
- Добавили data-source-utils в nab как отдельный модуль
- Небольшие исправления в nab-example
- Рефакторинг pom.xml 
```
