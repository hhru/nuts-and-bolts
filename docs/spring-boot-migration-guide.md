...
[//]: # (TODO: после финализации работ указать везде актуальные версии)
...
1. Обновить `common-pom` до версии `2.1.0.jakarta-rc3.spring-boot-rc1` или выше. В enforcer добавлено правило для проверки того, что в сервис не
   ...
   тянутся какие-то tomcat зависимости, необходимо их выпилить.
...
20. Вместо бина `Properties serviceProperties` (удален) использовать бины `InfrastructureProperties` и `Environment`/`ConfigurableEnvironment` (в том
    ...
    Если в вашем сервисе делается оверрайд бина `Properties serviceProperties`, его также необходимо выпилить (в том числе в тестах).
...
21. Вместо бина `Properties projectProperties` (удален) использовать бин `BuildProperties`.
    ...
    Если в вашем сервисе делается оверрайд бина `Properties projectProperties`, его также необходимо выпилить (в том числе в тестах).
...
22. Вместо бина `AppMetadata` (удален) использовать бины `InfrastructureProperties` и `BuildProperties`.
    ...
    Если в вашем сервисе делается оверрайд бина `AppMetadata`, его также необходимо выпилить (в том числе в тестах).

23. Выпилить получение пропертей из файла с помощью метода `PropertiesUtils.fromFilesInSettingsDir` (помечен как deprecated).
    ...
    п.18). `Properties` бины, при создании которых вызывается метод `PropertiesUtils.fromFilesInSettingsDir`, также необходимо выпилить.

    Пример бина, который необходимо выпилить:
...
24. На класс `HhAppProdConfig` повесить аннотацию `@Profile(Profiles.MAIN)` или `@MainProfile`. Таким образом, "prod" бины будут созданы только в том
    ...
    [//]: # (TODO: если после финализации всех работ получится избавиться в сервисах от Common и Prod конфигов, необходимо поправить данный пункт)
...
30. Поправить юнит тесты.
    ...
    можно выпилить, а аннотацию `@SpringBootTest` повесить на тестовые классы, которые от него наследуются.
    ...
    выпилить `HhAppCommonConfig.class`. В импорте данного класса нет необходимости, так как он уже импортируется в main class'е приложения (см. п.8).
    ...
    настройки добавляются в спринговый `Environment`. Если в ваших тестах не требуется оверрайд данных настроек, то их можно выпилить
    ...
    связанных с конфигурацией сервера) тоже можно выпилить.
...
[//]: # (TODO: поизучать какие еще есть настройки в буте для конфигурации веб приложений: webAppContext, servletContext, jettyServer и тд. Возможно в буте есть настройки, которых не было в набе, и в сервисах мб можно будет выпилить какие-то куски кода)
