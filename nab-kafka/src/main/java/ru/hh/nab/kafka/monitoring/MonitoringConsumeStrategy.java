...
public class MonitoringConsumeStrategy<T> implements ConsumeStrategy<T> {
  ...
  public MonitoringConsumeStrategy(
    ...
    this.timings = buildTimings(statsDSender, consumerDescription);
  ...
  private void addMdcData(List<ConsumerRecord<String, T>> messages) {
    ...
    MDC.put("topic", consumerDescription.getTopic());
    MDC.put("operation", consumerDescription.getOperation());
  ...
  private Timings buildTimings(StatsDSender statsDSender, ConsumerDescription identifier) {
