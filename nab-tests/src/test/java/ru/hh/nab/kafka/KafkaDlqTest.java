...
@Tag("integration")
public class KafkaDlqTest extends KafkaConsumerTestbase {
    ...
    KafkaConsumer<String> originalConsumer = startConsumer(
        ...
        (messages, ack) -> {
          for (ConsumerRecord<String, String> message : messages) {
            ...
            // if we won't ack current message it will result in infinite retries
            ack.nAcknowledge(message);
          }
  ...
  @Test
  public void testAckWithMalformedMessage() {
    AdminClient adminClient = kafkaTestUtils.getAdminClient();
    NewTopic originalTopic = createRandomTopic();
    adminClient.createTopics(List.of(originalTopic));
    ...
    kafkaTestUtils.sendMessage(originalTopic.name(), new ValidMessage(originalMessage));
    ...
    Assertions.assertEquals(1, receivedMessages.size());
    Assertions.assertEquals(new ValidMessage(originalMessage), receivedMessages.poll());

    // Tests will share kafka container, do not forget to perform resources cleanup
    originalConsumer.stop();
    adminClient.deleteTopics(List.of(originalTopic.name()));
  }
