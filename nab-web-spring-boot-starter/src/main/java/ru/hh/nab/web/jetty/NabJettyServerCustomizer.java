...
public class NabJettyServerCustomizer implements JettyServerCustomizer {
  ...
  private void configureConnectors(Server server) {
    Connector[] newConnectors = Arrays
        ...
        .map(connector -> {
          if (connector instanceof ServerConnector serverConnector) {
            ...
            return newServerConnector;
          } else {
            return connector;
          }
        })
        .peek(connector -> configureConnectionFactories(connector.getConnectionFactories()))
        .toArray(Connector[]::new);

    server.setConnectors(newConnectors);
  }
